...
void initKeyValuePairs( unsigned int length ) {
   ...
   keyValuePairs = (unsigned int*) kmalloc( keyValuePairsLength * sizeof(unsigned int), GFP_KERNEL );
...
static ssize_t dev_read(struct file *filep, char *buffer, size_t len, loff_t *offset){
   ...
   if (error_count==0){            // if true then have success
      printk(KERN_INFO "Comm: Sent %d to the user\n", value);
      return 0; 
   }
   else {
      printk(KERN_ALERT "Comm: Failed to send %d to the user\n", value);
...
static ssize_t dev_write(struct file *filep, const char *buffer, size_t len, loff_t *offset){
   ...
   if (error_count==0){            // if true then have success
      printk(KERN_INFO "Comm: Received %d from the user\n", value);
      return 0; 
   }
   else {
      printk(KERN_ALERT "Comm: Failed to send %d to the user\n", value);
